...
class Neural_Network(object):
    ...
    def train(self,batch_size, input, labels,x_test,y_test, num_epochs,learning_rate, filename):
        ...
        for i in range(num_epochs):
            ...
            self.check_accuracy('model.pkl',x_test,y_test)
    ...
    def forward_pass(self,input):
        ...
        self.layers[0].input = np.reshape(input,(1,input.shape[0]))
        ...
        for i in range(self.nlayer-1):
            ...
            self.layers[i+1].input = curr_temp

            if(self.layers[i+1].activation_function=="Sigmoid"):
                self.layers[i+1].input_activation = self.relu(curr_temp)
    ...
    def back_pass(self, labels):
        ...
        delta = self.cross_entropy_derivative(layer.input_activation,labels)
        ...
        for i in range(self.nlayer-2,0,-1):
            ...
            #print("jjojo")
            #print(delta.shape)
            #print(np.dot(delta,layer.weight.T).shape)
            #print(self.sigmoid(layer.input_activation,True).T.shape)
            delta = np.multiply(np.dot(delta,layer.weight.T), self.derivative_relu(layer.input))
            #print(delta.shape)
